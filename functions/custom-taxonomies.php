...
function render_teaching_methods_field( $field ) {
    $postID = get_acf_post_id();

    if(!is_admin() || is_null($postID))
        return;

    $techingMethods = (!empty(get_post_meta($postID, 'teachingMethods', false)[0])) ? get_post_meta($postID, 'teachingMethods', false)[0] : null;
    if(!is_null($techingMethods) && !empty($techingMethods))
    {
        echo '<div class="acf-preset">';
        echo '<p class="acf-preset-desc">Voreinstellung: </p><br/>';
        foreach ($field['choices'] as $value => $label) {
            if(in_array($value, $techingMethods))
            {
                echo '<div class="acf-preset-field">' . $label . '</div>';
            }
        }
...
add_action('acf/render_field/name=teachingMethods', 'render_teaching_methods_field');
