<?php /* Template Name: Fachportalinhalte-hotfix */
get_header();
global $post;
...
<div class="wlo-page">
        ...
        if (!empty($_FILES['fileToUpload']['name'])) {
            if ($_POST['consent']) {
                ...
                if (!empty($nodeID)) {
                    ...
                    if (callRepoApi($apiUrl, $fields, 'Content-Type: multipart/form-data') !== false) {
                        ...
                        $workflowComment = 'FÃ¼r folgende Sammlung vorgeschlagen: ' . $pageDiscipline . ' > ' . $pageTitle . ' > ' . $widgetName . ' (ID: ' . $collectionID . ')';
                        if (($_POST['ccby'])){
            ...
            if ( !empty($nodeID) ){
                ...
                if (callRepoApi($apiUrl, $data, 'Content-Type: application/json', 'PUT') !== false ){
                    ...
                    $headers[] = 'From: wirlernenonline.de <redaktion@wirlernenonline.de>';
                    $headers[] = 'Content-Type: text/html; charset=UTF-8';
                    $subject = 'Neuer Vorschlag fÃ¼r das Themenprotal: ' . $pageDiscipline . ' - ' . $pageTitle . ' - ' . $widgetName;
                    $body = $emailBody;
    ...
    <form action="<?php echo get_page_link($post->ID); ?>"  method=post enctype="multipart/form-data" class="portal_form">
        ...
        <div class="portal_form_file">
            ...
            <div class="portal_form_license">
                <div class="portal_form_licence_block">
                    ...
                    <textarea id="author" name="author" rows="4" placeholder="Wie soll die Urheber:in genannt werden, z.B.  Berta Beispielos fÃ¼r den Verein fÃ¼r Freie Inhalte"></textarea>
        ...
        <div class="portal_form_button">
            <input class="wlo-button" type=submit value="Absenden" name="addUrl" />
        </div>
